...
- **Provenance lines in recall/reflect output** — `hindsight_recall` now renders a compact provenance line per result (`[id: <id> | doc: <document_id or -> | ctx: <context snippet ≤80 chars or -> | scopes: <observation scopes ≤3 or ->]`) and `hindsight_reflect` appends a `Sources:` list (id + text snippet per source fact, ≤10 with a `(N more sources)` note) after the synthesized answer, so agents can judge a memory's trustworthiness instead of treating all recall as equally authoritative. Both tools gain a `provenance` param (default true; false strips the lines — the token-budget escape hatch, raw facts stay in `details`). Read-only server metadata; nothing is written. Known tradeoff: reflect provenance requests `includeFacts`, so the raw response (and its `based_on` evidence list) is larger — a necessary query-path addition since `based_on` exists only when requested.
- **`hindsight_curate` tool** — Exposes the server's memory-curation layer to agent turns: the fact-lifecycle write path. `find` (default) searches memories (`q` text search, optional `state` filter) and lists candidates one per line (id prefix, state, age, text snippet; token-budgeted with a `(truncated: N of M memories)` note); `invalidate` soft-retires a memory with a required `reason` (deliberate-only — the reason is recorded server-side and visible in the memory's history); `supersede` invalidates a memory naming its `successor` and queues the successor through the existing retain path (two-step, honestly reported when the retain step fails); `revert` restores an invalidated memory to valid. Gated by `toolsEnabled` (`"curate"`), hidden in degraded mode, and excluded from retention by the default `toolFilter`.
- **`hindsight_consolidate` tool** — Consolidation watchdog exposing the server's operations layer to agent turns. `status` lists pending/failed consolidation operations (one line each: id, status, age; token-budgeted with a `(truncated: N of M operations)` note); `trigger` starts consolidation with optional `observationScopes` (async, deduplicated server-side — reports the operation_id, does not poll); `recover` retries failed consolidations; `auto` recovers failed ops first, then triggers when pending exceeds `threshold` (default 5). Gated by `toolsEnabled` (`"consolidate"`), hidden in degraded mode, and excluded from retention by the default `toolFilter`.
