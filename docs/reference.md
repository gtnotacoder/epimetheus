...
| `toolsEnabled` | `true` | Which tools to register: `true` (all), `false` (none), or array of tool names (`"retain"`, `"recall"`, `"reflect"`, `"graph"`, `"consolidate"`, `"curate"`, `"set_extra_context"`, `"get_extra_context"`) |
...
```json
{
  "toolFilter": {
    ...
    "toolResult": { "exclude": ["grep", "find", "ls", "write", "edit", "hindsight_retain", "hindsight_recall", "hindsight_reflect", "hindsight_graph", "hindsight_consolidate", "hindsight_curate"] }
  }
}
```
...
When `toolsEnabled: true` (default), all tools are available. Set to an array of tool names (`"retain"`, `"recall"`, `"reflect"`, `"graph"`, `"consolidate"`, `"curate"`, `"set_extra_context"`, `"get_extra_context"`) to register only specific tools, or `false` to disable all tools.
...
| `hindsight_recall` | Search long-term memory using multi-strategy retrieval. Supports filtering by tags, memory types (`world`/`experience`/`observation`), and budget. Each result carries a provenance line (`id` / `document_id` / context snippet / observation scopes) so a fact's source can be judged; `provenance: false` strips the lines to save tokens. |
| `hindsight_reflect` | Generate a synthesized answer from long-term memory. Unlike recall which returns raw facts, reflect uses the bank's identity, mental models, and multi-step reasoning to produce a contextual markdown answer. Best for questions requiring synthesis of multiple memories. By default the answer is followed by a `Sources:` list (id + text snippet per source fact) so the synthesis is traceable to its inputs; `provenance: false` skips the source-facts request. |
...
| `hindsight_consolidate` | Consolidation watchdog: check or act on the server's consolidation state. `status` lists pending/failed consolidation operations; `trigger` starts consolidation (async and deduplicated server-side — reports the operation_id, does not poll); `recover` retries failed consolidations; `auto` recovers failed ops first, then triggers when pending exceeds `threshold`. |
| `hindsight_curate` | Curate the memory bank's fact lifecycle. `find` (default) searches memories (`q` text search, optional `state` filter) and lists candidates one per line; `invalidate` soft-retires a memory with a required `reason` (recorded server-side); `supersede` invalidates a memory naming its `successor` and queues the successor for storage (two-step); `revert` restores an invalidated memory to valid. |
